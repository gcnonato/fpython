...
class Agenda:
    ...
    def __init__(self, master):
        ...
        self.frame1 = Frame(
            master, width=1000, height=25, bg="#222125", bd=1, relief="raise"
        )
        ...
        self.e_id = Entry(
            master, width=10, font=("arial 10 bold"), bd=1, relief="solid"
        )
        self.e_id.place(x=50, y=45)
        self.la_id = Label(master, text="ID:", font=("arial 9 bold"))
        self.la_id.place(x=30, y=45)

        self.e_cliente = Entry(
            master, width=40, font=("arial 12 bold"), bd=1, relief="solid"
        )
        ...
        self.e_tel = Entry(
            master, width=15, font=("arial 12 bold"), bd=1, relief="solid"
        )
        ...
        self.e_func = Entry(
            master, width=25, font=("arial 12 bold"), bd=1, relief="solid"
        )
        ...
        self.e_hora = Entry(
            master, width=10, font=("arial 12 bold"), bd=1, relief="solid"
        )
    ...
    def add_cliente(self, *args, **kwargs):
        ...
        if (
            ...
            sql = (
                "INSERT INTO agend("
                "Nome, Telefone, Celular, Email, Funcionario, Serviço, Hora, Data"
                ")VALUES (?,?,?,?,?,?,?,?)"
            )
            self.conn.getCursor().execute(
                sql,
                (
                    self.cliente,
                    self.tel,
    ...
    def atualizar(self, *args, **kwargs):
        ...
        sql = (
            "UPDATE agend SET nome=?, telefone=?, celular=?, email=?, funcionario=?, serviço=?, hora=?, data=?"
            "WHERE id=?"
        )
        self.conn.getCursor().execute(
            sql,
