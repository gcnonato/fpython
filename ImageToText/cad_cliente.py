...
class Clientes:
    ...
    def add_clientes(self, *args, **kwargs):
        ...
        if (
            ...
            sql = (
                "INSERT INTO client("
                "nome, telefone, celular, email, endereço, bairro, cidade, identidade, cpf, nascimento"
                ")VALUES (?,?,?,?,?,?,?,?,?,?)"
            )
            c.execute(
                sql,
    ...
    def atualizar(self, *args, **Kwargs):
        ...
        query = (
            "UPDATE client SET nome=?, telefone=?, celular=?, email=? endereço=?, "
            "bairro=?, cidade=?, identidade=?, cpf=?, nascimento=?"
        )
        c.execute(
            query,
            (
