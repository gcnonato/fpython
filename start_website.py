...
def get_root_xml_data():
    ...
    if uname and pword is None:
        ...
        try:
            ...
            auth_resp = requests_retry_session().get(
                auth_url, timeout=3, headers=header
            )
            ...
            root_resp = requests_retry_session().get(
                root_url, timeout=3, headers=header
            )
...
class ReutersVideoRequest:
    ...
    def update_database(self, test=False, outfile=None, videos=False):
        ...
        try:
                        ...
                        for result in channels_result:
                            ...
                            if items_resp.status_code == 200:
                                ...
                                description = (
                                    ...
                                    + "\n"
                                    + "\n".join(
                                        re.compile(r"<p>(.+)</p>").findall(items_str)
                                    )
                                )
                                video_url = f"{part_video_url}?token={auth_token}"
                                tags = ",".join(
                                    re.compile(r"<keyword>(.*)</keyword>").findall(
                                        items_str
                                    )
                                ).lower()
                                thumbnail = (
                                    re.compile(
                                        "http://[a-z].*BASEIMAGE:[0-9]+X[0-9]+"
                                    ).findall(items_str)[1]
                                    + "?token={auth_token}"
                                )
                                recordingDate = re.compile(
                                    r"<firstCreated>(.*)</firstCreated>"
                                ).findall(items_str)[0]
                                status = "New"
                                # Store to DB
                                sql_data = (
                                ...
                                if outfile is not None:
                                    ...
                                    #     "recordingDate",
                                    # ]
                                    # success_write = self.DB.write_to_csv(
                                    #     outfile=outfile, header=headers, row=csv_data
                                    # )
            ...
            if debug_email:
                ...
                # mail_logging.send_mail(
                #     "New request for videos", "{}\n\nGetRT works !".format(debug_email)
                # )
            # Close DB Connection
            self.DB.close()
...
def second_script():
    logging.basicConfig(
        filename=os.path.dirname(os.path.realpath(__file__)) + "/reuters-3rdScript.log",
        filemode="a+",
...
def initialize_upload(youtube, options):
    ...
    body = dict(
        ...
        recordingDetails=dict(recordingDate=options.recordingDate),
    )
    ...
    while response is None:
        try:
            ...
            if response is not None:
                if "id" in response:
                    logging.warning(
                        "Link youtube https://www.youtube.com/watch?v={} was successfully uploaded.".format(
                            response["id"]
                        )
                    )
                    global ytUrl
...
def download_video(my_video):
    ...
    try:
        ...
        logging.error(
            "The video can't be downloaded from link: {}".format(my_video["videoUrl"])
        )
        exit()
...
def upload_video(my_video):
    ...
    my_decription = (
        cut_text(my_video["description"].decode("utf8"), 4200, ".") + std_description
    )
    ...
    my_tag = (
        my_video["id"]
        + ","
        + my_video["tags"].decode("utf8")
        + ",コロナウイルス,reuters,business,finance,news,politics,top news"
    ...
    argparser.add_argument(
        "--description", help="Video description", default=my_decription
    )
    argparser.add_argument("--category", default="25", help="Numeric video category.")
    argparser.add_argument(
        "--keywords", help="Video keywords, comma separated", default=my_tag
    )
    argparser.add_argument(
        "--privacyStatus",
    ...
    argparser.add_argument(
        "--recordingDate",
        default=my_video["recordingDate"],
        help="The value is specified in ISO " "8601 format.",
    )
    ...
    try:
        ...
        logging.warning(
            "Successfuly uploaded video with id: {}, ytUrl:{}, title:{}".format(
                my_video["id"], ytUrl, my_video["title"]
            )
        )
        # mail_logging.send_mail(
        #     "Successfuly upload new video",
        #     "{}, {}, {}".format(my_video["id"], ytUrl, my_video["title"]),
        # )
        ...
        #         my_video["id"], e.resp.status, e.content
        #     ),
        # )
        logging.error(
...
def set_thumbnail(my_video, args=None):
    ...
    try:
        filename = (
            os.path.dirname(os.path.realpath(__file__)) + "/" + thumb_name + ".jpg"
        )
        ...
        logging.critical("Error code: ", e.code)
        logging.error(
            "The thumbnail can't be downloaded from link: {}".format(thumbnail_url)
        )
        exit()
    except URLError as e:
        logging.critical("We failed to reach a server.")
    ...
    if os.path.exists(filename):
        try:
            ...
            #     videoId=youtube_video_id, media_body=MediaFileUpload(filename)
            # )
...
def choose_yt_channel(my_video):
    ...
    all_channels = dict(
        {
            "arabic": arabic_channel,
            "english": english_channel,
            "german": german_channel,
            "japan": japan_channel,
            "spanish": spanish_channel,
            "russian": russian_channel,
        }
    )

    for key, value in all_channels.items():
        if my_video["channel"] in value:
            credentials_static_path = (
                os.path.dirname(os.path.realpath(__file__)) + "/oauth-channels/"
            )
...
def upload():
    logging.basicConfig(
        filename=os.path.dirname(os.path.realpath(__file__)) + "/reuters-youtube.log",
        filemode="a+",
        format="%(name)s - %(levelname)s - %(message)s",
    )
    mail_logging = ""  # MailLogging("asherswing@gmail.com")
    ...
    CLIENT_SECRETS_FILE = (
        os.path.dirname(os.path.realpath(__file__)) + "/client_secret.json"
    )
...
def last_fifty_videos():
    ...
    cursor.execute(
        "SELECT id, guid, version, dateCreated, source, language, mediaType, \
...
def index():
    ...
    for i in row:
        tableHtml = tableHtml + render_template(
            "table.html",
            id=i[0].split("_", 1)[1],
            version=i[1],
            datecreated=i[2],
