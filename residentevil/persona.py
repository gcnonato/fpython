...
class Persona:
    ...
    def get_basic_info(self):
        ...
        resultado = (
            self.bs_obj.find("div", class_="td-page-content").find("p").findAll("em")
        )
        data = {
            e.text.split(":")[0].strip(" "): e.text.split(":")[1].strip(" ")
            for e in resultado
        }

        if len(data) != 4:
            resultado = (
                self.bs_obj.find("div", class_="td-page-content")
                .find("p")
                .text.split("\n")
            )
            data = {
    ...
    def get_biography(self):
        try:
            ...
            resultado = get_value_mark(
                self.bs_obj, "strong", "Biografia e Participação"
            )[0]
