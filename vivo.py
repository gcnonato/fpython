...
url = "https://login.vivo.com.br/loginmarca/" \
      "appmanager/marca/publico?origem=https://login.vivo.com.br/saml2/idp/sso/login-return#"
...
class VivoWithSelenium:
    ...
    def percorrer_tabela(self, pagina, first_page_previous, _f):
        ...
        print(
            f"Primeira Página Anterior..: {first_page_previous}\n"
    ...
    def main(self, url, params):
        fileout = f"vivo3g-{datetime.today().date()}-" \
                  f"{datetime.today().time().isoformat().split('.')[0].replace(':','_')}.txt"
        if os.name != "posix":  # Windows
...
def load_txt():
    ...
    for info in file_read:
        ...
        ws[cell_A].font = Font(name="Arial", size=14, color="00000000")
        ws[cell_A].fill = PatternFill(fgColor=WHITE, fill_type="solid")
        ws[cell_B].font = Font(name="Arial", size=14, color="00000000")
        ws[cell_B].fill = PatternFill(fgColor=WHITE, fill_type="solid")
        if "2020" in info:
            ws[cell_A] = info.strip()
            # Testa se o dia do mês é 6 e se for o 6 e a primeira vez, entra
            if (
                datetime.strptime(info.split(",")[0], "%d/%m/%Y").day == 6
                and first_time
            ):
                first_time = False
                qt_mensal = 3000
            ...
            ws[cell_A] = info.split("->")[
                0
            ]  # é colocado Gastei..: 7% OU Restante..: 93%
            if "MB" in info:
