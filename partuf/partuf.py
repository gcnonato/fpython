    ...
    for tabela in range(0, len(tabelas)):
        ...
        try:
            ...
            for quali in range(0, len(html_qualidades)):
                resolut.append(
                    f"{html_qualidades[quali].string} {strong.string}    |||     {html_tamanho[quali].string}"
                )
    ...
    if len(magnetico) == 0:
        for tabela in range(0, len(tabelas)):
            ...
            try:
                for quali in range(0, len(html_qualidades)):
                    resolut.append(
                        f"{html_num_epi[quali].string} {html_qualidades[quali].string}"
                    )
    ...
    if index == "":
        subprocess.Popen(
            [
                ...
                mag_final,
                "--path",
                os.path.join("%USERPROFILE%", "Desktop", "Filmes"),
            ],
            shell=True,
        )
    else:
