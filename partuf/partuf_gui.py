...
class Partuf:
    ...
    def layout_selecionar_filmes(self):
        layout = \
            [
                [
                    sg.Listbox(self.titulos, size=(60, 18), font='Arial 18')
                ],
                [
                    sg.OK()
                ]
        ...
        for tabela in range(0, len(self.tabelas)):
            ...
            try:
                ...
                for quali in range(0, len(html_qualidades)):
                    self.resolut.append(f"{html_qualidades[quali].string} {strong.string}")
    ...
    def lista_magneticos_da_serie_selecionada(self):
        if len(self.magnetico) == 0:
            for tabela in range(0, len(self.tabelas)):
                ...
                for quali in range(0, len(html_qualidades)):
                    self.resolut.append(f"{yellow(html_num_epi[quali].string.replace('Ep.', '-'))} {orange('->>')} {html_qualidades[quali].string} {strong.string}")
    ...
    def layout_selecionar_resolucao(self):
        ...
        layout = \
            [
                [
                    sg.Listbox(self.resolut, size=(60, 18), font='Arial 18')
                ],
                [
                    sg.OK()
                ]
    ...
    def opcoes_de_uso(self):
        ...
        if self.escolha == 0:
            ...
            try:
                ...
                start = subprocess.Popen(['peerflix', mag_final], shell=True)
            except FileNotFoundError as err:
                print(f'Error.: {err}\n{self.escolha}')

        elif self.escolha == 2:
            print(f"\nLink magnético:\{mag_final}\n")

if __name__ == '__main__':
    pu = Partuf()
    fire.Fire(pu.option())
    while True:
        pu.mostrar_lista_filmes()  # Essa função chama outra função chamada "url_scrape(), a "url_scrape" chama a "layout_inicial()".
        if len(pu.titulos):
